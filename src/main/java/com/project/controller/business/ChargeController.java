...
public class ChargeController extends BaseController {
	...
	public void index() throws Exception {
		...
		String sSelect = "select uc.*, u.name user_name, u.img_url user_img_url, u.user_name user_user_name, u.user_mobile user_user_mobile";
		String sWhere = " from db_user_charge uc left join db_user u on uc.user_id=u.id where uc.business_id=? and uc.status=?";
		params.add(getLoginBusinessId());
		params.add(UserCharge.STATUS_YIFUKUAN);
		if (StrKit.notBlank(getPara("uid"))) {
			sWhere += " and uc.user_id=?";
		...
		if (StrKit.notBlank(getPara("startT")) && StrKit.notBlank(getPara("endT"))) {
			...
			Object account = Db.findFirst(
					...
					UserCharge.STATUS_YIFUKUAN, getLoginBusinessId(),
			...
			Object account = Db.findFirst(
					...
					UserCharge.STATUS_YIFUKUAN, getLoginBusinessId()).get("number");
			setAttr("account", CodeUtil.getNumber(Float.parseFloat(account.toString())));
			// 累计充值笔数
