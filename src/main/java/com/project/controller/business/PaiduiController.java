...
public class PaiduiController extends BaseController {
	...
	public void index() throws Exception {
		...
		String sSelect = "select p.*, u.name user_name, u.img_url user_img_url, tt.title tables_type_title, s.title shop_title";
		String sWhere = " from db_paidui p left join db_user u on p.user_id=u.id left join db_tables_type tt on p.tables_type_id=tt.id left join db_shop s on p.shop_id=s.id where 1=1";
	...
	public void template() throws Exception {
		...
		Business business = Business.dao.findById(getLoginBusinessId());
	 ...
	 */
	public void doTemplate() throws Exception {

		Business business = Business.dao.findById(getLoginBusinessId());
		business.set("template_id_paidui", getPara("template")).update();
