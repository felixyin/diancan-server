...
public class UserController extends BaseController {
	...
	public void index() throws Exception {
		...
		String sSelect = "select u.*"
				+ ",(select ifnull(sum(grand_total), 0) from db_orders where user_id=u.id and display=1 and closed=0 and status=9 and business_id=u.business_id) orders_amount";
		...
		if (StrKit.notBlank(getPara("startT")) && StrKit.notBlank(getPara("endT"))) {
			// 会员数量
			Object user_number = User.dao.findFirst(
					...
					User.STATUS_SHANCHU, getLoginBusinessId(),
			...
			Object xiaofei_number = User.dao.findFirst(
					...
					User.STATUS_SHANCHU, getLoginBusinessId(), Orders.DISPLAY_YES, Orders.CLOSED_NO,
					Orders.STATUS_FINISH, getLoginBusinessId(),
					DateUtil.formatDate(getParaToDate("startT"), "yyyy-MM-dd HH:mm"),
			...
			Object xiaofei_number = User.dao.findFirst(
					...
					User.STATUS_SHANCHU, getLoginBusinessId(), Orders.DISPLAY_YES, Orders.CLOSED_NO,
					Orders.STATUS_FINISH, getLoginBusinessId()).get("number");
			setAttr("xiaofei_number", xiaofei_number);
		}
		render("list.htm");
	}
	...
	public void changeStatus() throws Exception {
		...
		if (!user.get("business_id").toString().equals(getLoginBusinessId())) {
			setAttr("success", false);
			setAttr("msg", "没有操作权限");
			renderJson();
			return;
		}
		user.set("status", getPara("status")).update();
